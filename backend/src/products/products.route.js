...
router.get("/", async (req, res) => {
  try {
    ...
    console.error("Error fetching all product ");
    res.status(500).send({ message: "Error fetching all product" });
  }
});
...
router.patch(
  ...
  async (req, res) => {
    try {
      ...
      res.status(500).send({ message: "failed updating the product" });
    }
  }
);
